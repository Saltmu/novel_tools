---
description: Run the entire novel review pipeline (Formatting -> Parallel Review Skills) in parallel across multiple files and save the results in `reviews`.
---
...
1. Create a subdirectory for the results within `reviews` named after the target file (e.g., `reviews/[TARGET_FILE_BASENAME]`).
```bash
mkdir -p reviews/[TARGET_FILE_BASENAME]
```
...
```bash
echo "Agent: Please run novel-formatter on [TARGET_FILE] and save to reviews/[TARGET_FILE_BASENAME]/01_formatted.txt. Wait for completion of this file's formatting."
```

2.5. Run the keyword-based RAG filter script to generate a compact settings context file.
```bash
poetry run python src/filter_context.py reviews/[TARGET_FILE_BASENAME]/01_formatted.txt reviews/[TARGET_FILE_BASENAME]/01_filtered_context.txt
```
...
- **Logic Auditor (Subagent)**: Runs the `text-reviewer-logic` skill on `01_formatted.txt` using `01_filtered_context.txt` as settings reference. Saves findings to `reviews/[TARGET_FILE_BASENAME]/02_logic_consistency.yaml`.
- **Style Editor (Subagent)**: Runs the `text-reviewer-style` skill on `01_formatted.txt`. Saves findings to `reviews/[TARGET_FILE_BASENAME]/03_style_expression.yaml`.
...
- **Consolidated Output:** Save the merged findings as `reviews/[TARGET_FILE_BASENAME]/00_integrated_findings.yaml`. Optionally, create a clean markdown summary report at `reviews/[TARGET_FILE_BASENAME]/00_integrated_report.md`.
...
4. **Interactive Reflective Review (Chat/CLI):**
   ...
   ```bash
   poetry run apply-findings --dir reviews/[TARGET_FILE_BASENAME] --interactive
   ```
...
5. **Apply Selection (Automated via Script):**
   ...
   - To apply specific IDs selected by the user:
     ```bash
     poetry run apply-findings --dir reviews/[TARGET_FILE_BASENAME] --accept-ids INT-001,INT-003
     ```
   - To apply all findings marked as `accepted: "y"` (if the user edited the YAML file manually):
     ```bash
     poetry run apply-findings --dir reviews/[TARGET_FILE_BASENAME] --auto
     ```
