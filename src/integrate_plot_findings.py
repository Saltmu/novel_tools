...
def run_integration_llm(output_dir, target_text, raw_findings_text, model):
    """Calls PlotFindingsIntegrationTask to merge and resolve conflicts in the plot findings."""
    print(f"Sending plot consolidation request to AgyClient ({model})...")
...
def _collect_raw_findings(output_dir: str) -> list[dict]:
    ...
    print(f"Found {len(yaml_files)} plot YAML files to integrate.")
...
def integrate_plot_findings_in_dir(output_dir, plot_filepath, model):
    ...
    if not os.path.exists(plot_filepath):
        print(f"Error: Plot file '{plot_filepath}' does not exist.", file=sys.stderr)
        return False
