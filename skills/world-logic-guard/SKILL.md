...
# 手順
0. 最新データの同期: ターミナルで `./venv/bin/python3 sync_gdrive.py` を実行し、Google Driveから最新の設定情報を取得する。
1. 入力されたテキストから、世界観に依存する要素（魔法的現象、長距離移動、特殊な生物など）を特定する。
2. `data/sources/` 内の `01_設定資料集.txt` を参照し、それらの要素に関する「絶対的なルール」を抽出する。
3. テキストの描写が、世界の法則を「無視」または「改変」していないか比較検証する。
4. 矛盾がある場合、以下の形式で出力する。

## 【設定矛盾アラート】
- **該当箇所:** 「（テキストの抜粋）」
- **違反している設定:** （例：ネフェスの消耗制限について）
- **解説:** この世界の法則では〇〇であるため、この描写は不自然です。
- **修正案:** 設定を維持しつつ物語を成立させるための代替案を提示。
