...
# 手順
0. 最新データの同期: ターミナルで `poetry run python src/sync_gdrive.py` を実行し、Google Driveから最新の設定情報を得ること。
1. `data/sources/` 内の最新のプロットファイル（例：`*第1幕プロット*.txt`）等の全体構成データを参照し、物語の「ゴール（オチ・問題解決手段）」を把握する。
