...
# 手順
0. 最新データの同期: ターミナルで `poetry run python src/sync_gdrive.py` を実行し、Google Driveから最新の設定情報を取得する。
1. 入力されたテキストから、主要なキーワード（人名、地名、重要なアイテム）を抽出する。
2. `data/sources/` 内の歴史年表（例：`*創世記から現代まで*.txt`）、キャラクター概要（例：`*キャラクター概要*.txt`）、プロット（例：`*第1幕プロット*.txt`）などのファイルから関連する記述を検索する。
