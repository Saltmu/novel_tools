...
# 手順
0. **最新データの同期:** ターミナルで `poetry run python src/sync_gdrive.py` を実行し、Google Driveから最新の設定情報（設定資料集、キャラクター概要、プロットなど）を取得する。
1. 入力されたテキストから、世界観に依存する要素、主要な登場人物、キーアイテム、重要な出来事を特定する。
2. `data/sources/` 内の設定資料集（例：`*設定資料集*.txt`）、キャラクター概要（例：`*キャラクター概要*.txt`）、プロット（例：`*プロット*.txt`）などのファイルから関連する記述を検索・抽出する。
